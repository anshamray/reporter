...
| Duration | 🕐  **${humanizeDuration(
    ...
    if (passedTests.length) {
      body += `\n<details>\n<summary><h3>✅ Passed Tests (${passedTests.length})</h3></summary>\n\n`;
      body += passedTests.map(t => {
        let text = `- ${fullName(t)} (${humanizeDuration(parseFloat(t.run_time), { maxDecimalPoints: 0 })})`;
        if (t.retry > 0) {
          text += ` Passed on retry: ${t.retry + 1}`;
        }
        return text;
      }).join('\n');
      body += '\n\n</details>\n';
    ...
    try {
      ...
      // Signal completion to GitHub Actions
      this.signalCompletion(url);
    } catch (err) {
      console.log(APP_PREFIX, chalk.yellow('GitHub'), `Couldn't create GitHub report ${err}`);
      // Signal completion even if there's an error
      this.signalCompletion();
    }
  }

  signalCompletion(reportUrl = '') {
    if (process.env.GITHUB_ACTIONS === 'true') {
      // Set an output variable for GitHub Actions
      console.log(`::set-output name=testomatio_report_url::${reportUrl}`);
