...
class SlackPipe {
  ...
  addTest(test) {
    ...
    console.log('SlackPipe received test:', JSON.stringify({
      title: test.title,
      status: test.status,
      retries: test.retry,
      ...
      fullTest: test,
      willBeRetried: test.willBeRetried,
      // This will show all properties
    }, null, 2)); */
  ...
  async finishRun(runParams) {
    ...
    if (failedTests.length) {
      ...
      body += failedTests.map(t => {
        let text = `*${fullName(t)}`;
        text += ` (${humanizeDuration(parseFloat(t.run_time), { maxDecimalPoints: 0 })})*`;
        ...
        const lastStep = getLastNonZeroStep(t.stack);
        if (lastStep) {
          text += `\`Failed at Step: ${lastStep}\`\n`;
        }
