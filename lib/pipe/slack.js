...
function getLastNonZeroStep(message) {
  ...
  console.log('Total lines:', lines.length); // Log the total number of lines

  // Find where the steps start
  const stepsStartIndex = lines.findIndex(line => line.includes(STEPS_MARKER));
  console.log('Steps start index:', stepsStartIndex); // Log the index of the steps marker
  ...
  for (let i = stepsStartIndex + 1; i < lines.length; i++) {
    ...
    console.log('Match found:', match); // Log the match result
